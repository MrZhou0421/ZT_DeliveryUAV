...
namespace ego_planner
{
                                        ...
                                        Eigen::Vector3d local_target_vel, bool flag_polyInit, bool flag_randomPolyTraj)
  {
    ...
    if (pp_.drone_id <= 0)
    {
      ...
      std::cout << "\033[1;32m[DP Time Allocator] Executing Energy-Aware Time Reallocation...\033[0m\n";
      Eigen::MatrixXd optimal_control_points;
      
      flag_step_2_success = applyDPTimeAllocation(pos, start_end_derivatives, ts, optimal_control_points);
      ...
      {
        std::cout << "\033[1;31m[DP Time Allocator] Failed. Fallback to default check.\033[0m\n";
                                                  ...
                                                  const std::vector<Eigen::Vector3d> &waypoints, const Eigen::Vector3d &end_vel, const Eigen::Vector3d &end_acc)
  {
    ...
    double dist_thresh = max(total_len / 8, 5.0);
  ...
  bool EGOPlannerManager::applyDPTimeAllocation(UniformBspline &traj, vector<Eigen::Vector3d> &start_end_derivative, double &ts_out, Eigen::MatrixXd &optimal_control_points)
  {
    if (!dp_time_allocator_) return false;
    ...
    for (double t = 0.01; t <= total_duration; t += 0.01)
    {
      Eigen::Vector3d p = traj.evaluateDeBoorT(t);
      if ((p - last_pt).norm() >= ds)
      {
        waypoints.push_back(p);
        last_pt = p;
      }
    }
    Eigen::Vector3d end_pt = traj.evaluateDeBoorT(total_duration);
    if ((end_pt - waypoints.back()).norm() > 0.05)
    {
      waypoints.push_back(end_pt);
    }

    // 2. Compute DP Profile
    double best_energy, dp_total_time;
    vector<double> opt_vels;
    if (!dp_time_allocator_->computeOptimalTime(waypoints, best_energy, dp_total_time, opt_vels))
    {
      ROS_WARN("DP Time Allocator failed. Fallback to uniform.");
      return false;
    }

    // 3. Resample trajectory uniformly in time to reconstruct Bspline
    double dt_uniform = 0.05; 
    vector<Eigen::Vector3d> point_set;
    
    vector<double> waypoint_times;
    waypoint_times.push_back(0.0);
    for (size_t i = 0; i < waypoints.size() - 1; i++)
    {
      double v_entry = i < opt_vels.size() ? opt_vels[i] : dp_time_allocator_->getVMax() * 0.1;
      double v_exit = i + 1 < opt_vels.size() ? opt_vels[i + 1] : 0.1;
      double v_avg = (v_entry + v_exit) / 2.0;
      if (v_avg < 1e-6) v_avg = 0.1;
      
      double seg_time = (waypoints[i+1] - waypoints[i]).norm() / v_avg;
      waypoint_times.push_back(waypoint_times.back() + seg_time);
    }
    
    // Add decel time at the end
    double v_final = opt_vels.back();
    double a_max = dp_time_allocator_->getAMax();
    double decel_time = a_max > 0 ? v_final / a_max : 0.0;
    waypoint_times.back() += decel_time; 
    dp_total_time = waypoint_times.back();
    
    for (double t = 0.0; t <= dp_total_time + 1e-4; t += dt_uniform)
    {
      // find which segment t falls into
      int seg_idx = 0;
      for (size_t i = 0; i < waypoint_times.size() - 1; i++) {
        if (t >= waypoint_times[i] && t <= waypoint_times[i+1]) {
          seg_idx = i;
          break;
        }
      }
      if (t >= waypoint_times.back()) seg_idx = waypoints.size() - 2;
      
      double t_ratio = 1.0;
      if (waypoint_times[seg_idx+1] > waypoint_times[seg_idx]) {
        t_ratio = (t - waypoint_times[seg_idx]) / (waypoint_times[seg_idx+1] - waypoint_times[seg_idx]);
      }
      t_ratio = std::max(0.0, std::min(1.0, t_ratio));
      
      Eigen::Vector3d pt = waypoints[seg_idx] * (1 - t_ratio) + waypoints[seg_idx+1] * t_ratio;
      point_set.push_back(pt);
    }
    
    ts_out = dt_uniform;
    UniformBspline::parameterizeToBspline(ts_out, point_set, start_end_derivative, optimal_control_points);

    // Optimize refinement for feasibility on B-spline format if DP gave something jerky
    bspline_optimizer_->ref_pts_.clear();
    for (size_t i = 0; i < point_set.size(); i++) {
        bspline_optimizer_->ref_pts_.push_back(point_set[i]);
    }
    bool refine_success = bspline_optimizer_->BsplineOptimizeTrajRefine(optimal_control_points, ts_out, optimal_control_points);
    if (!refine_success) {
        ROS_WARN("[DP Time Allocator] Refine trajectory failed but we continue.");
    }
