...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    ////////////////////////
    fpv_bool_pub=nh.advertise<std_msgs::String>("fpv/string",100);
    ////////////////////////
  ...
  void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    ...
    bool has_global_time = ros::param::get("/mission_global_start_time", global_start_sec);
    
    // 记录规划开始的绝对时间
    ros::Time time_plan_start = ros::Time::now();
    bool success = false;
    ...
    ros::Time time_plan_end = ros::Time::now();
    if (success)
    {
      end_pt_ = next_wp;
  ...
  void EGOReplanFSM::odometryCallback(const nav_msgs::OdometryConstPtr &msg)
  {
    ...
    odom_pos_(1) = msg->pose.pose.position.y;//change
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    // std_msgs::String msg_fpv;
    //       cnt_fpv++;
    //       std::string str=std::to_string(cnt_fpv);
    //       msg_fpv.data="ok"+str;
    //       fpv_bool_pub.publish(msg_fpv);
    //       cout<<"msg_fpv.data"<<msg_fpv.data<<endl;
