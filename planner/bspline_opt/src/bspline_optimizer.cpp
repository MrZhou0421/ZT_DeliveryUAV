...
namespace ego_planner
{
  ...
  void BsplineOptimizer::setParam(ros::NodeHandle &nh)
  {
    ...
    nh.param("optimization/lambda_turn", lambda_turn_, 0.005);
  ...
  void BsplineOptimizer::calcEnergyTurnCost(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient)
  {
    cost = 0.0;
    if (lambda_turn_ <= 0.0) return;

    double v_ref = max_vel_; 
    double mass = 2.614 + 0.255; 
    double weight = lambda_turn_ * mass * pow(v_ref, 3.0);
    
    Eigen::Vector3d acc, temp_acc;
    for (int i = 0; i < q.cols() - 2; i++)
    {
      acc = q.col(i + 2) - 2 * q.col(i + 1) + q.col(i);
      cost += weight * acc.squaredNorm();
      
      temp_acc = 2.0 * weight * acc;
      gradient.col(i + 0) += temp_acc;
      gradient.col(i + 1) += -2.0 * temp_acc;
      gradient.col(i + 2) += temp_acc;
    }
...
//#define USE_SECOND_CLEARENCE_CHECK
  ...
  void BsplineOptimizer::combineCostRebound(const double *x, double *grad, double &f_combine, const int n)
  {
    ...
    double f_smoothness, f_distance, f_feasibility /*, f_mov_objs*/, f_swarm, f_terminal, f_turn;
    ...
    Eigen::MatrixXd g_turn = Eigen::MatrixXd::Zero(3, cps_.size);
    ...
    calcEnergyTurnCost(cps_.points, f_turn, g_turn);

    f_combine = lambda1_ * f_smoothness + new_lambda2_ * f_distance + lambda3_ * f_feasibility + new_lambda2_ * f_swarm + lambda2_ * f_terminal + f_turn;
    //f_combine = lambda1_ * f_smoothness + new_lambda2_ * f_distance + lambda3_ * f_feasibility + new_lambda2_ * f_mov_objs;
    //printf("origin %f %f %f %f\n", f_smoothness, f_distance, f_feasibility, f_combine);

    Eigen::MatrixXd grad_3D = lambda1_ * g_smoothness + new_lambda2_ * g_distance + lambda3_ * g_feasibility + new_lambda2_ * g_swarm + lambda2_ * g_terminal + g_turn;
  ...
  void BsplineOptimizer::combineCostRefine(const double *x, double *grad, double &f_combine, const int n)
  {
    ...
    /* ---------- evaluate cost and gradient ---------- */
    double f_smoothness, f_fitness, f_feasibility, f_turn;
    ...
    Eigen::MatrixXd g_turn = Eigen::MatrixXd::Zero(3, cps_.points.cols());
    ...
    f_combine = lambda1_ * f_smoothness + lambda4_ * f_fitness + lambda3_ * f_feasibility + f_turn;
    // printf("origin %f %f %f %f\n", f_smoothness, f_fitness, f_feasibility, f_combine);

    Eigen::MatrixXd grad_3D = lambda1_ * g_smoothness + lambda4_ * g_fitness + lambda3_ * g_feasibility + g_turn;
    memcpy(grad, grad_3D.data() + 3 * order_, n * sizeof(grad[0]));
  }
